    ...
    hostPlatform <-
        f
            $ option
                ...
                ( long "host-platform"
                    <> metavar "PLATFORM"
                    <> help "The platform of the system this image will run on"
                )
    keyboard <-
        f
            $ strOption
                (long "keyboard" <> metavar "KEYBOARD" <> help "The keyboard layout to use on the installation media")
...
askHostPlatform
    :: (IOE :> es, Concurrent :> es, TypedProcess :> es, Log :> es, Reader AppState :> es) => Eff es System
askHostPlatform = onHost Localhost do
    currentSystem <- hostSystem
    allSystems :: [System] <-
...
askOpts
    ...
    => MediaOpts Maybe -> MediaOpts (Eff es)
askOpts opts =
    MediaOpts
